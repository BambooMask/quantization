...
class Resnet20(nn.Module):
    def __init__(self, block, quan_first_last=False, constr_activation=None):
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, quan_first_last=False, constr_activation=None):
        super(ResNet, self).__init__()
        self.quan_first_last = quan_first_last
        self.quan_activation = constr_activation is not None
        self.constr_activation = constr_activation

        if self.quan_first_last:
...
def resnet18(quan_first_last=False, constr_activation=None, preactivation=False):
    block = PreActivationBlock if preactivation else BasicBlock
    model = ResNet(block, [2, 2, 2, 2], quan_first_last=quan_first_last, constr_activation=constr_activation)
    return model


def resnet20(quan_first_last=False, constr_activation=None, preactivation=False):
    block = PreActivationBlock if preactivation else BasicBlock
    model = Resnet20(block, quan_first_last, constr_activation)
    return model


def resnet50(quan_first_last=False, constr_activation=None, preactivation=False):
    block = Bottleneck
    model = ResNet(block, [3, 4, 6, 3], quan_first_last=quan_first_last, constr_activation=constr_activation)
    return model
